...
function optim_function(SRange, FRange, optim_struct::OptimStruct, args...; maxrnaLC = 10, maxrnaFC = 60, freeparametersidx =[x for x in eachindex(SRange)], fixedparameters =[-1], multipledistribution=false, kwargs...)
    ...
    if multipledistribution
        ...
        if data.datagroup == FixedCellData()
            ...
            mrna_distribution_model = bfparameters[end-1]*bfparameters[end].*convolution(distribution_mrna(optim_struct_wrapper.model, bfparameters[1:4], optim_struct_wrapper.maxrnaFC)) .+ bfparameters[end-1]*(1-bfparameters[end]).*convolution(distribution_mrna(optim_struct_wrapper.model, vcat(bfparameters[5:7],bfparameters[4]), optim_struct_wrapper.maxrnaFC))
...
function ini_optim_multipleDistribution(optim_struct::OptimStruct, datagroup::FixedCellData; kwargs...)
    function err_func(params,optim_struct_wrapper::OptimStructWrapper)
        parameters = utiles.mergeparameter_base(optim_struct_wrapper.fixedparam, params, optim_struct_wrapper.freeparametersidx)
        estimate_signal = parameters[end-1]*parameters[end].*convolution(distribution_mrna(optim_struct_wrapper.model, parameters[1:4], optim_struct_wrapper.maxrnaFC)) .+ parameters[end-1]*(1-parameters[end]).*convolution(distribution_mrna(optim_struct_wrapper.model, vcat(parameters[5:7],parameters[4]), optim_struct_wrapper.maxrnaFC))
